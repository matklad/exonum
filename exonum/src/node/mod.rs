...
pub struct NodeApiConfig {
    ...
    /// Set CORS part of response header for browser-based clients.
...
impl AllowOrigin {
    fn as_middleware(&self) -> CorsMiddleware {
        match *self {
            AllowOrigin::Any => CorsMiddleware::with_allow_any(),
            AllowOrigin::Whitelist(ref hosts) => CorsMiddleware::with_whitelist(
...
pub fn create_public_api_handler(
    ...
    let cors_middlware = config.allow_origin.as_ref().map(
        AllowOrigin::as_middleware,
    );
    let mut chain = Chain::new(mount);
    if let Some(middlware) = cors_middlware {
        chain.link_around(middlware);
    }
    chain
}